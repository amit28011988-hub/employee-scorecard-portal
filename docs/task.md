# Task Checklist: Employee Scorecard Portal

## Planning & Setup
- [ ] Create Implementation Plan <!-- id: 1 -->
- [ ] Initialize Next.js Project (App Router) <!-- id: 2 -->
- [ ] Set up Supabase Project & Database Schema <!-- id: 3 -->

## Core Features
- [ ] **Authentication** <!-- id: 4 -->
    - [ ] Create Login Page (Name/ID + PIN)
    - [ ] Implement Admin Login (secure route)
- [ ] **Admin Dashboard** <!-- id: 5 -->
    - [ ] Create Excel Upload Component
    - [ ] Implement CSV/Excel Parsing Logic
    - [ ] Employee Management (View list, Change PIN)
- [ ] **Employee Dashboard** <!-- id: 6 -->
    - [ ] Scorecard Display UI (Matches the requested aesthetic)
    - [ ] Month Selection/History
    - [ ] PDF Export Functionality

## Verification & Polish
- [ ] Verify Calculations vs Excel data <!-- id: 7 -->
- [ ] Test Mobile Responsiveness <!-- id: 8 -->
- [ ] Final UI Polish (Premium/Glassmorphism style) <!-- id: 9 -->
